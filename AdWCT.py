...
def wct_core(cont_feat, styl_feat, weight=1, registers=None, device='cpu'):
    ...
    styl_min = styl_feat.min()
    styl_max = styl_feat.max()
    styl_mean = torch.mean(styl_feat, 1).unsqueeze(1).expand_as(styl_feat)
    s_mean = torch.mean(styl_feat, 1)
    styl_feat -= styl_mean


    if not registers:
        _, c_e, c_v = svd(cont_feat, iden=True, device=device) #content UV Decomposition
        ...
        EDE = torch.mm(torch.mm(s_v[:, 0:k_s], torch.diag(s_d) * weight), (s_v[:, 0:k_s].t()))    #Es

        if registers is not None:
            registers['EDE'] = EDE
            registers['s_mean'] = styl_mean
            registers['c_v'] = c_v
            registers['c_e'] = c_e
    else:
        EDE = registers['EDE']
        styl_mean = registers['s_mean']
    ...
    step2 = torch.mm(step1, (c_v[:, 0:k_c].t()))     # Ec

    whiten_cF = torch.mm(step2, cont_feat)
    whiten_sF = torch.mm(EDE, styl_feat)

    whiten_cF_mean = torch.mean(whiten_cF, 1).unsqueeze(1).expand_as(whiten_cF)
    whiten_sF_mean = torch.mean(whiten_sF, 1).unsqueeze(1).expand_as(whiten_sF)

    targetFeature = adaptive_instance_normalization(cont_feat, whiten_sF)

    #
    # targetFeature = torch.mm(EDE, whiten_cF)
    targetFeature = targetFeature + s_mean.unsqueeze(1).expand_as(targetFeature)
    targetFeature.clamp_(cont_min.item(), cont_max.item())
...
def calc_mean_std(feat, eps=1e-5):
    ...
    feat_mean = feat.view(N, -1).mean(dim=1).view(N, 1)
